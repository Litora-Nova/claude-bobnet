...
// avatarMode ist bewusst ein String, kein boolean-Prop: Vue castet ein FEHLENDES
// boolean-Prop automatisch zu `false` ("Boolean casting"), das wuerde die
// Initialen-Kachel bei jedem Alt-Aufrufer (z. B. der PO-Karte im Inbox-Hub) faelschlich
// erzwingen. Als String bleibt "nicht gesetzt" wirklich `undefined` → Bild bleibt Default.
const props = defineProps<{ agent: any; tag?: string; showAvatar?: boolean; avatarSize?: number; avatarMode?: 'image' | 'initials' }>()
...
// Team-Karten/Team-Ansicht (PO 2026-07-19): nur der Lead behaelt das Bild, alle
// anderen zeigen Initialen statt Foto (nie ein Emoji — bleibt gewahrt, Initialen
// sind Typografie, kein Glyph). `avatarMode` bleibt undefined bei Alt-Aufrufern
// (z. B. der PO-Karte im Inbox-Hub) → dort unveraendertes Bild-Verhalten.
...
<template>
  <div class="me-card">
    <div class="ava" v-if="showAvatar !== false" :style="{ width: avaPx, height: avaPx }">
      <template v-if="avatarMode === 'initials'">
        <span class="initials" :style="{ width: avaPx, height: avaPx, fontSize: `${Math.round((avatarSize || 52) * 0.36)}px` }">{{ initials }}</span>
      </template>
      <template v-else>
    ...
    <div class="who">
      ...
      <div v-if="agent.model" class="model">{{ agent.model }}</div>
    </div>
...
.ava .initials { display: flex; align-items: center; justify-content: center; border-radius: 10px; background: #21262d; border: 1px solid #30363d; color: #c9d1d9; font-weight: 700; letter-spacing: .02em; }
...
.model { font-size: 11px; color: #6e7681; margin-top: 2px; max-width: 260px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
