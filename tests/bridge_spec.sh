...
t "M4: Multibyte über 4KB (Bytes) trotz <4096 Zeichen → REJECT 2" "2" \
  "$(SSH_ORIGINAL_COMMAND="$umlaut_msg" recv codex-2 >/dev/null 2>&1; echo $?)"
...
t "M3: ACCEPT-Audit kaputt → exit 3 (fail-closed, nicht REJECT 2)" "3" \
  "$(SSH_ORIGINAL_COMMAND='[alpha]@Bill: sollte nicht ankommen' DEV_TEAM_REGISTRY="$tmp/registry.json" BOBNET_PEERS="$tmp/peers.json" BRIDGE_LOG="$BROKENLOG" bash "$RECV" codex-2 >/dev/null 2>&1; echo $?)"
...
t "L7: Peer mit Steuerzeichen/Sonderzeichen → REJECT 2" "2" \
  "$(SSH_ORIGINAL_COMMAND='[alpha]@Bill: hi' DEV_TEAM_REGISTRY="$tmp/registry.json" BOBNET_PEERS="$tmp/peers.json" BRIDGE_LOG="$LOG" bash "$RECV" "$(printf 'codex-2\nFAKE | ACCEPT | peer=evil')" >/dev/null 2>&1; echo $?)"
...
t "send: Remote-REJECT (rc 2) propagiert als 2" "2" \
  "$(BOBNET_PEERS="$tmp/peers.json" BRIDGE_TEST_MODE=1 BRIDGE_TRANSPORT_CMD="exit 2" bash "$SEND" codex-2 "[alpha] hi" >/dev/null 2>&1; echo $?)"
...
PATH="$tmp/bin:$PATH" BOBNET_PEERS="$tmp/peers.json" BRIDGE_TRANSPORT_CMD="cat > $tmp/should-not-exist.txt" \
  bash "$SEND" codex-2 "[alpha]@Bill: M5 override ignoriert" >/dev/null 2>"$tmp/m5-warn.txt"
...
BOBNET_PEERS="$tmp/peers.json" BRIDGE_TEST_MODE=1 BRIDGE_TRANSPORT_CMD="DEV_TEAM_REGISTRY=$tmp/registry.json BOBNET_PEERS=$tmp/peers.json BRIDGE_LOG=$LOG bash $RECV codex-2" \
...
BOBNET_PEERS="$tmp/peers.json" BOBNET_SEND_LOG="$SENDLOG" BRIDGE_TEST_MODE=1 BRIDGE_TRANSPORT_CMD="exit 0" \
  bash "$SEND" codex-2 "[alpha]@Bill: send-audit test" >/dev/null
ok "L8: Sender-Audit-Datei geschrieben" test -s "$SENDLOG"
ok "L8: Sender-Audit nennt Peer" grep -q 'peer=codex-2' "$SENDLOG"
...
BOBNET_PEERS="$tmp/peers.json" BOBNET_SEND_LOG="$SENDLOG" BRIDGE_TEST_MODE=1 BRIDGE_TRANSPORT_CMD="exit 2" \
  bash "$SEND" codex-2 "[alpha]@Bill: send-audit zweite Zeile" >/dev/null 2>&1
...
STANDUP_DIR="$tmp/standup2" BOBNET_PEERS="$tmp/peers.json" BRIDGE_TEST_MODE=1 BRIDGE_TRANSPORT_CMD="exit 0" \
  bash "$SEND" codex-2 "[alpha] via standupdir" >/dev/null
...
env -u STANDUP_DIR BOBNET_PEERS="$tmp/peers.json" BRIDGE_TEST_MODE=1 BRIDGE_TRANSPORT_CMD="exit 0" \
  bash "$SEND" codex-2 "[alpha] ohne standupdir" >/dev/null
