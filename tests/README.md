...
| `bin/who-owns` | `who_owns_spec.sh` | **#7 Zuständigkeits-Query** (Black-Box gegen mktemp-Fixture-Registry via `BOBNET_REGISTRY`): owns[]-Pfad-Match exakt + Unterpfad, spezifischster owns gewinnt (Reason nennt den tiefsten Pfad) · project-`path`-Match · keyword in responsibility/name/uid · kein Treffer → exit 1 + klare Meldung · `--list` zeigt alle Projekte · fehlende Registry → exit 2 · Diskriminierungs-Guards (fremder Tenant wird NICHT getroffen — non-tautologisch) |
| `schemas/registry.schema.json` | `registry_schema_spec.sh` | **#7 Schema-Conformance** gegen mktemp-Fixtures mit einem inline, self-contained draft-07-SUBSET-Validator (pur python3 stdlib — KEINE ajv/jsonschema-Dep, dep-arm; skip-grün ohne python3): konformer Eintrag → VALID · malformed rejected (fehlende uid, bad status-enum, additionalProperty-Typo, bad uid-pattern, owns-Dupe/uniqueItems, version-als-String) · Validator-Sanity-Gegenprobe (akzeptiert/ablehnt beide Richtungen) · mitgeliefertes `projects.registry.example.json` ist konform (skip-grün, wenn nicht gefunden) |
